...
var stepProgressMiddleware = require('./middleware/step-progress');
...
var BaseController = function BaseController(options) {
  ...
  this.use(stepProgressMiddleware(options));
};
...
BaseController.prototype.getNextStep = function getNextStep(req, res) {
  ...
  next = _.reduce(forks, function forkFold(result, value) {
    ...
    if (value.invalidateNextIfPass && conditionSatisfied) {
      this.invalidate(this.options.next, req.sessionModel);
    }

    if (value.invalidateTargetIfFail && !conditionSatisfied) {
      this.invalidate(value.target, req.sessionModel);
    }
...
BaseController.prototype.invalidate = function invalidate(stepName, sessionModel) {
...
BaseController.prototype.locals = function controllerLocals(req, res) {
  ...
  var stepNumber = req.sessionModel.get('stepData').stepNumber;
  var totalSteps = req.sessionModel.get('stepData').totalSteps;
